...
class Annotation
  attr_accessor :x_px, :y_px, :w_px, :h_px, :motivation, :text, :user, :anno_id, :canvas, :tags, :target_start, :target_end, :svg, :style
  ...
  def self.from_oa(json_hash, canvas)
    ...
    anno.style = json_hash['stylesheet']['value'] if json_hash.keys.include? 'stylesheet'
